...
interface LegislationLibraryPageProps {
  params: Promise<{
    lang: "en" | "ar";
  }>;
}
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return generatePageMetadata("legislation_library", lang as "en" | "ar", "legislation-library");
}

export default async function LegislationLibraryPage(props: LegislationLibraryPageProps) {
  const params = await props.params;
  const { lang } = params;
  const dictionary = await getDictionary(lang);
  const { legislation_library: hubData } = dictionary;
  const schemas = getPageJsonLd("legislation_library", lang, "legislation-library");
      ...
      <section className="bg-background py-16 md:py-24">
        <div className="container mx-auto px-4 max-w-4xl space-y-8">
          <h2 className="text-3xl md:text-4xl font-bold mb-8 text-foreground leading-tight">
            {lang === 'ar' ? hubData.intro.heading : hubData.intro.heading_en}
          </h2>
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {hubData.categories.map((cat: any, idx: number) => {
              ...
              const CardBody = (
                <Card className={cn(
                  "h-full border-border transition-all duration-300 flex flex-col relative overflow-hidden group/card",
                  isReady
                    ? "hover:border-primary/50 hover:shadow-lg bg-card hover:-translate-y-1"
                    : "bg-muted/50 cursor-not-allowed"
                )}>
                  <div className={cn(
                    "absolute top-0 end-0 p-6 opacity-5 transition-transform duration-500",
                    isReady
                      ? "group-hover/card:opacity-10 group-hover/card:scale-110"
                      : ""
                  )}>
                    <Icon className={cn(
                      "w-32 h-32",
                      ...
                      {isReady ? (
                        <div className="flex items-center text-primary opacity-80 group-hover/card:opacity-100 transition-opacity">
                          ...
                          <ArrowRight className={cn(
                            ...
                            lang === 'ar'
                              ? 'rotate-180 group-hover/card:-translate-x-1'
