...
import { LineChart, Landmark, Globe, ArrowRight, BookOpen } from "lucide-react";
import Link from "next/link";
import { cn } from "@/lib/utils";
import { BoldText } from "@/components/BoldText";

interface BICenterPageProps {
  params: Promise<{
    lang: "en" | "ar";
  }>;
}
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return generatePageMetadata("bi_center", lang as "en" | "ar", "bi-center");
}

export default async function BICenterPage(props: BICenterPageProps) {
  const params = await props.params;
  const { lang } = params;
  const dictionary = await getDictionary(lang);
  const { bi_center: hubData } = dictionary;
  const schemas = getPageJsonLd("bi_center", lang, "bi-center");
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {hubData.categories.map((cat: any, idx: number) => {
              ...
              return isReady ? (
                <Link key={idx} href={`/${lang}/bi-center/${cat.slug}`} className="h-full block">
                  {CardBody}
                </Link>
              ) : (
